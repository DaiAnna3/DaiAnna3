    ...
    def __init__(self, opt):
        ...
        self.generate = nn.Sequential(
           ...
           #input = (n, c, h, w = 256, 100, 1, 1)
            ...
            # decon = (input - 1)*stride + k - 2*padding = (32-1)*2 + 4-2 = 64
            ...
            # decon = (input - 1)*stride + k - 2*padding = (64-1)*2 + 4-2 = 128


            nn.ConvTranspose2d(in_channels=self.feats, out_channels=3, kernel_size=4, stride=2, padding=1, bias=False),

            nn.Tanh(),
            # decon = (input - 1)*stride + k - 2*padding = (128-1)*2 + 4-2 = 256
            #output = (n, c, h, w = 256, 3, 256, 256)
...
class Dnet(nn.Module):
    def __init__(self, opt):
        ...
        self.discrim = nn.Sequential(
            ...
            # con = (input - k + 2 * padding ) / stride +  1 = (128 - 4 + 2) / 2 + 1 = 64


            nn.Conv2d(in_channels= self.feats * 2, out_channels= self.feats * 4, kernel_size= 4, stride= 2, padding= 1,bias=False),
            nn.BatchNorm2d(self.feats * 4),
            nn.LeakyReLU(0.2, True),
            # con = (input - k + 2 * padding ) / stride +  1 = (64 - 4 + 2) / 2 + 1 = 32


            nn.Conv2d(in_channels= self.feats * 4, out_channels= self.feats * 8, kernel_size= 4, stride= 2, padding= 1, bias=False),
            nn.BatchNorm2d(self.feats *8),
            nn.LeakyReLU(0.2, True),
            # con = (input - k + 2 * padding ) / stride +  1 = (32 - 4 + 2) / 2 + 1 = 16


            nn.Conv2d(in_channels=self.feats * 4, out_channels=self.feats * 8, kernel_size=4, stride=2, padding=1,
                      bias=False),
            nn.BatchNorm2d(self.feats * 8),
            nn.LeakyReLU(0.2, True),
            # con = (input - k + 2 * padding ) / stride +  1 = (16 - 4 + 2) / 2 + 1 = 8

            nn.Conv2d(in_channels=self.feats * 4, out_channels=self.feats * 8, kernel_size=4, stride=2, padding=1,
                      bias=False),
            nn.BatchNorm2d(self.feats * 8),
            nn.LeakyReLU(0.2, True),
            # con = (input - k + 2 * padding ) / stride +  1 = (8 - 4 + 2) / 2 + 1 = 4


            nn.Conv2d(in_channels= self.feats * 8, out_channels= 1, kernel_size= 4, stride= 1, padding= 0, bias=True),
            # con = (input - k + 2 * padding ) / stride +  1 = (4 - 4 + 0) / 1 + 1 = 1
...
for epoch in range(opt.max_epoch):  #总循环轮次
    ...
    torchvision.utils.save_image(vid_fake_image.data[:16], "%s/%s.png" % (opt.result_save_path, epoch), normalize=True) #保存前16幅图像
